...
Basic screenshot script with QR Scanner feature. Good for screenshot keybindings.
It should be noted that the cursor isn't included in screenshots.
In the **--select-cp** mode, if there's a QR Code detected in the picture, it will try to scan it.
If it fails, it will leave the screenshot copied on the clipboard.
If it succeeds it will prompt to ask if you want the QR Code result on your clipboard.
...
--window
: Screenshot selected window, and save it as a file

--window_cp
: Screenshot selected window, and put it on the clipboard without saving
...
: If a QR Code is detected in the screenshot, it will try to scan it.
: If the scan fails, it will leave the screenshot on the clipboard.
: if the scan succeeds you will be prompted to copy either the image or the scanned result.
